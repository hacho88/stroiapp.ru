...
# Check oc_opensetting columns
r = post("db/select", {"sql": "SELECT * FROM oc_opensetting LIMIT 1"})
print(f"Setting columns: {list(r['rows'][0].keys()) if r.get('rows') else 'no rows'}")

# Add settings with backtick-quoted group
settings = [
...
for key, value in settings:
    ...
    if r3.get('rows', []):
        ...
        sql = f"INSERT INTO oc_opensetting (store_id, `group`, `key`, `value`, serialized) VALUES (0, 'stroiapp_logistics', '{key}', '{value}', 0)"
        try:
...
# Verify
r6 = post("db/select", {"sql": "SELECT * FROM oc_opensetting WHERE `group` = 'stroiapp_logistics'"})
