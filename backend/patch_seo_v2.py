...
# The blog routing block to insert (as base64)
blog_block = """
...
# Simple patcher using base64
patch_php = f"""<?php
class ControllerApiPatchSeo2 extends Controller {{
    public function index() {{
        ...
        // Find the geo routing block
        $geo_marker = "Geo page routing";
        $geo_pos = strpos($content, $geo_marker);
        $r[] = "Geo marker pos: " . ($geo_pos !== false ? $geo_pos : "not found");

        if ($geo_pos === false) {{
            // Find foreach loop instead
            $foreach_pos = strpos($content, "foreach ($parts as $part)");
            ...
            // Find return; after geo block
            $geo_return = strpos($content, "return;", $geo_pos);
            // Find closing }} of geo if block
            $insert_pos = strpos($content, "}}", $geo_return) + 2;
        }}

        $blog_code = base64_decode("{blog_b64}");
        $new_content = substr($content, 0, $insert_pos) . $blog_code . substr($content, $insert_pos);
...
print("=== Writing patcher v2 ===")
r1 = post("file/writeSafe", {"path": "catalog/controller/api/patch_seo2.php", "content": patch_php})
print(f"  {r1}")

time.sleep(1)
print("\n=== Patching ===")
req = urllib.request.Request(f"https://{ip}/index.php?route=api/patch_seo2", headers={"Host": "stroiapp.ru"})
try:
